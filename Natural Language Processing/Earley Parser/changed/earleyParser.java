...
{
	//The grammar!
	//static GrammarImproved grammar;
	...
	*/
	...
	{
                argv= new String[]{"/Users/nupurbisht/NetBeansProjects/hw2/src/prob-simple.gr"};
		if((argv.length != 1) && (argv.length != 2))
		{
			...
			if(argv.length == 1)
			{
				...
				{
					System.out.print("?");
					temp = br.readLine(); // reading the senetnce...
					arr = temp.split(" "); // spliting by space...
					Production result; // initialising result as production... 
					...
					if(grammar.canParse(arr))
					{
						if((result = grammar.parse(arr)) != null)
						{
							...
							//float probs[]= new float[50];
                                                        result.recursivePrint();// printing the parse tree....
                                                        //result.probabilityPrint();
                                                        //result.addprobabilities(0);
