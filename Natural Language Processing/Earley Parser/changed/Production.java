...
public class Production 
{
	...
	{
		if(left != p.left || right.length != p.right.length || dot != p.dot || probability != p.probability)
			return false;
	...
	public void recursivePrint()
	{
		...
		System.out.print(" ]");
               // System.out.print(total_prob +"\n");
               // return total_prob;
                
	}
        
        /*public void probabilityPrint() throws IOException
	{
		...
		for(int i =0; i<right.length; i++){
			if(this.children!=null){
				this.children[i].probabilityPrint();
                             
			}
			else{
				for(int j=0; j< right.length; j++){
					//System.out.print(right[j]);
                                        //addprobabilities(this.probability);
        ...
        public void addprobabilities(float p)
        {
            while(true)
            {
                total_prob[k]=p;
